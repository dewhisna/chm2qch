...
{
    QSettings s;
    s.setValue("writeRoot", writeRoot);
    s.setValue("generate",  generate);
    s.setValue("clean",     clean);
    s.setValue("qtDir",     qtDir);
    s.setValue("nameSpace", nameSpace);
    s.setValue("destDir",   destDir);
}

void Converter::loadSettings()
{
    QSettings s;
    writeRoot = s.value("writeRoot", true ).toBool();
    generate  = s.value("generate",  true ).toBool();
    clean     = s.value("clean",     false).toBool();
    qtDir     = s.value("qtDir").toString();
    destDir   = s.value("destDir").toString();
    //nameSpace = s.value("nameSpace").toString();
}
...
void Converter::writeQhp(const QString &filename, ChmFile *chm, const QString &nameSpace, bool writeRoot)
{
    ...
    xml.writeComment(QString("Generated by chm2qch from %1").arg(QFileInfo(fileName).fileName()));
